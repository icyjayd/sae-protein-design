...
repo_root = os.path.abspath(os.path.join(os.path.dirname(__file__), ".."))
sys.path.insert(0, os.path.join(repo_root, "interplm"))

from interplm.sae.inference import load_sae_from_hf

from sae.utils.esm_utils import encode_sequence
from sae.utils.grade_reconstructions import grade_pair, mean_grade
import warnings
...
def test_multiple_sequences():
    ...
    for i, seq in enumerate(sequences[:samples]):
        print(f"\n--- Testing sequence {i+1}/{samples} ---")
